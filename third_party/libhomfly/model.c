                     ...
                     weave  *oldweave)
{
  ...
  this_pos = (old_going_in[first]) ? list[second] : list[first];
  that = (old_going_in[first]) ? first : second;
  for (t = model[that]; t != 0; t = t->z) t->self = this_pos;
  if (model[this_pos] != 0)
  {
    for (t = model[this_pos]; t->z != 0; t = t->z) ;
    t->z = model[that];
  }
  else model[this_pos] = model[that];
  model[that] = 0;

  list[this_pos]       = list[that];
  list[list[this_pos]] = this_pos;
  ...
  if (this_pos == first || this_pos == second)
  {
  ...
  else
  {
    ...
    this_pos = map[this_pos];
    m_correct(model, this_pos);
  }
